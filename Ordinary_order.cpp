...
Ordinary_order::Ordinary_order(Date order_date, Time order_time, string description, Client client):Order(order_date, order_time)
{
    this->client = client;
    this->description = description;

    // price cannot be less than zero
    if (this->client.get_all_sum() >= 0)
    {
        this->price = this->client.get_all_sum();
        number = ++amount;
    }
